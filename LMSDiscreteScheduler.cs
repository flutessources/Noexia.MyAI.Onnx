...
namespace Noexia.MyAI.Onnx
{
    public class LMSDiscreteScheduler : SchedulerBase
    {
               ...
               int order = 4)
        {
            ...
            var dimensionsArray = sample.Dimensions.ToArray();
            var derivativeItems = new DenseTensor<float>(dimensionsArray);
            ...
            var revDerivatives = Enumerable.Reverse(this.Derivatives);
            ...
            var prevSample = TensorHelper.AddTensors(sampleArray, sumTensor.ToArray(), dimensionsArray);
