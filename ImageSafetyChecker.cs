...
namespace StableDiffusion.ML.OnnxRuntime
{
    ...
    {
        public static bool IsNotSafe(Tensor<float> resultImage, EExecutionProvider executionProvider, int deviceIndex, int width, int height, string safetyModelPath)
        {
            var sessionOptions = SessionOptionsFactory.CreateSession(executionProvider, deviceIndex);

            //clip input
            var inputTensor = ClipImageFeatureExtractor(resultImage, width, height);
            //images input
            ...
            var session = new InferenceSession(safetyModelPath, sessionOptions);
        ...
        private static DenseTensor<float> ClipImageFeatureExtractor(Tensor<float> imageTensor, int width, int height)
        {
            // Read image
